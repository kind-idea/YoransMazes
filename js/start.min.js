...
camera.position.set(0,-60,100);camera.lookAt(center);renderer.setSize(c,d);b.appendChild(renderer.domElement);b=new THREE.AmbientLight(4210752);scene.add(b);var e=[];a(0,0,50,.9);a(-45,-45,50,.5);a(45,45,-50,.2);b=!0===useWebGL?[["tex/c3.jpg","groundTex"],["tex/side.jpg","boxTex"],["tex/metA.jpg","metATex"],["tex/metB.jpg","metBTex"]]:[["tex/c3small.jpg","groundTex"],["tex/sidesmall.jpg","boxTex"],["tex/metA.jpg","metATex"],["tex/metB.jpg","metBTex"]];for(c=0;c<b.length;c++)nTexs[b[c][1]]=THREE.ImageUtils.loadTexture(b[c][0])}
...
detGeo=new THREE.BoxGeometry(9.2,9.2,9.2);detMatA=new THREE.MeshLambertMaterial({color:8918579,transparent:!0,opacity:.4});detMatB=new THREE.MeshLambertMaterial({color:13209,transparent:!0,opacity:.4});crate=new THREE.Mesh(new THREE.BoxGeometry(9,9,9),new THREE.MeshLambertMaterial({map:nTexs.boxTex}));scene.add(charMesh);scene.add(doorClose);scene.add(doorOpen);mainMenu()}function render(){renderer.render(scene,camera)}var gameData={},toggles={},keycode={},useWebGL;
...
function load(){JSONLoader=new THREE.JSONLoader;document.getElementById("loading").style.display="block";var a=THREE.ImageUtils.loadTexture("mods/door/norm.jpg"),b=THREE.ImageUtils.loadTexture("mods/door/spec.jpg"),c=THREE.ImageUtils.loadTexture("mods/door/map.jpg"),d=THREE.ImageUtils.loadTexture("mods/door/mapred.jpg"),e=new THREE.MeshPhongMaterial({ambient:new THREE.Color(.64,.64,.64),specular:new THREE.Color(0,0,0),color:new THREE.Color(.64,.64,.64),map:c,normalMap:a,specularMap:b,shininess:50}),
f=new THREE.MeshPhongMaterial({ambient:new THREE.Color(.64,.64,.64),specular:new THREE.Color(0,0,0),color:new THREE.Color(.64,.64,.64),map:d,normalMap:a,specularMap:b,shininess:50});JSONLoader.load("mods/door/doorClose.js",function(){return function(a){a.faceVertexUvs[1]=a.faceVertexUvs[0];doorClose=new THREE.Mesh(a,f);doorCloseDec=new THREE.Mesh(a,e);loadedModels+=1;doneCheck()}}());JSONLoader.load("mods/door/doorOpen.js",function(){return function(a){a.faceVertexUvs[1]=a.faceVertexUvs[0];doorOpen=
new THREE.Mesh(a,e);loadedModels+=1;doneCheck()}}());JSONLoader.load("mods/guy/guy.js",function(){return function(a){var b=new THREE.MeshPhongMaterial({ambient:new THREE.Color(.9,.9,.9),specular:new THREE.Color(0,0,0),color:new THREE.Color(.9,.9,.9),map:THREE.ImageUtils.loadTexture("mods/guy/tex.png"),normalMap:THREE.ImageUtils.loadTexture("mods/guy/norm.png"),specularMap:THREE.ImageUtils.loadTexture("mods/guy/spec.png"),shininess:50});a.faceVertexUvs[1]=a.faceVertexUvs[0];charMesh=new THREE.Mesh(a,
b);loadedModels+=1;doneCheck()}}())}
...
a.faceVertexUvs[0];for(var e=0;e<d.length;e++){var f=new THREE.MeshPhongMaterial({ambient:new THREE.Color(.8,.8,.8),specular:new THREE.Color(0,0,0),color:new THREE.Color(.7,.7,.7),map:d[e],normalMap:b,lightMap:c}),f=new THREE.Mesh(a,f);barCloners.push(f)}loadedDecor+=1;doneCheck()}}());for(var a=["mods/table/dirty.jpg","mods/table/rough.jpg","mods/table/weathered.jpg"],f=THREE.ImageUtils.loadTexture("mods/table/norm.jpg"),h=THREE.ImageUtils.loadTexture("mods/table/spec.jpg"),g=THREE.ImageUtils.loadTexture("mods/table/ao.jpg"),
k=[],e=0;e<a.length;e++)k.push(THREE.ImageUtils.loadTexture(a[e]));JSONLoader.load("mods/table/tGeo.js",function(){return function(a){a.faceVertexUvs[1]=a.faceVertexUvs[0];for(var b=0;b<k.length;b++){var c=new THREE.MeshPhongMaterial({ambient:new THREE.Color(.64,.64,.64),specular:new THREE.Color(1,1,1),color:new THREE.Color(.64,.64,.64),map:k[b],normalMap:f,lightMap:g,specularMap:h,shininess:50}),c=new THREE.Mesh(a,c);tabCloners.push(c)}loadedDecor+=1;doneCheck()}}())}
